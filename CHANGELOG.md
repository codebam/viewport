...
- Bindings carry eight more flags, written as pseudo-modifiers the way `locked`
  ...
  the key is held, `long_press+` fires only after a half-second hold,
  `universal+` (or `submap_universal+`) matches in every binding mode, and
  `click+` / `drag+` fire a button binding on the release depending on whether
  the pointer moved past `drag_threshold`. The two halves of a chord do not
  shadow each other, so one chord can do one thing on press and another on
  release.
...
- `cursor.hide_on_key_press` puts the cursor away on the next keystroke and
  ...
  `cursor.hide_on_touch` does the same for a finger on a touch screen.
  `keyboard.rules` and `keyboard.model` name the xkb rules file and keyboard
  model beside the layout, variant and options that were already there.
...
- Window rules match Hyprland's vocabulary beyond `app_id` and `title`:
  ...
  new `view.tearing` and `view.idle_inhibit` messages.
...
- Dependencies are current. The lockfile catches up under its existing
  requirements, and the five crates.io dependencies whose major version had
  moved are bumped: `ureq` 2 to 3 (its API is `http`-crate shaped now, and its
  three per-phase timeouts are one end-to-end one), `base64` 0.22 to 0.23,
  ...
  because 0.6.0 made it an optional feature.
...
[Unreleased]: https://github.com/codebam/viewport/compare/v0.3.0...HEAD
[0.3.0]: https://github.com/codebam/viewport/releases/tag/v0.3.0

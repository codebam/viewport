...
| `launcher.list` | `apps[]` with `id`, `name`, optional `icon` (a `data:` URL, already resolved from the installed themes) and optional `detail` (the entry's keywords, or its main category) — the applications the launcher can start, in answer to `launcher.query`. The scan is the compositor's: the shell is a web page and cannot read `XDG_DATA_DIRS`. The command line itself never crosses the wire; `launcher.launch` names an `id` and the compositor starts what it scanned |
...
| `launcher.launch` | `id` (from the last `launcher.list`) — start that application. The process is handed an xdg-activation token minted for it, so the window that appears opens focused rather than behind whatever the user moved on to. An `id` from a list a later query replaced is refused |

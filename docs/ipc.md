...
| `osk.wanted` | `wanted` — the client with keyboard focus just enabled or disabled a `zwp_text_input_v3`, i.e. asked for (or stopped asking for) an on-screen keyboard. Sent only on the edge, and only as accurately as `sync_osk_wanted` in `input.rs` can notice one without a dedicated callback — see its doc comment. The reference shell raises and lowers its own keyboard on this; a shell with a real keyboard attached is free to ignore it |

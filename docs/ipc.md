...
| Message | Payload |
| --- | --- |
| `config` | `layout` (a built-in or declared extension), optional `layout_extensions[]` of `{name,url}` with local paths already resolved, optional `widget_extensions[]` of `{name,url}` with local paths already resolved (the scripts a `custom` bar widget loads by name, so a widget named in `bar_widgets` or `bar_items` as `type: custom` has something to run), `logo`, `tutorial`, `binds[]` of `chord`, `action` and optional `mode` — the keymap as the compositor will really act on it, in the order it matches; optional `bar`, `bar_blur` (whether the floating `auto` bar blurs the windows behind it; always present, and absent from the config file means on), optional `rules[]`, optional `theme{}`, optional `wallpaper` (the desktop background: a URL, already resolved and encoded, a CSS value such as `#1a1b26` or a gradient, or a video URL the shipped shell plays as an animated wallpaper) and `wallpaper_mode` (`fill`, `fit`, `stretch`, `center` or `tile`; absent is `fill`; a video in `tile` fills the screen, since a film cannot repeat), optional `clock{locale,hour12,format}` (how the bar writes the clock and the calendar under it — a BCP 47 tag, a twelve/twenty-four-hour choice, and a strftime-style template; absent means the shell uses the locale its engine runs under, which is the one thing the compositor cannot name for it), `osk` (`"auto"`, `"manual"` or `"off"` — whether the on-screen keyboard may raise itself, and whether `Mod4+Shift+k` still works if not; always present, unlike most of this message, since the compositor always has some answer for it), and `dark_mode` (whether applications are being told to draw themselves dark; always present, for the same reason `osk` is, and painted by nothing in the shell — it is the settings panel's switch reading its own position) |
...
| `view.added` | `id`, `title`, `app_id`, optional stable xdg-toplevel `tag`, `output` (name of the output it opened on), `replay`, `floating`, compositor-owned `minimized`, `width`, `height`, `min_width`, `min_height`, `parent` when this window is a dialog of another, and optional nearest-first `ancestors` containing only view IDs whose native Wayland process ancestry was verified by the compositor. Process IDs never cross this protocol |
...
| `status.update` | `cpu`, `memory`, `load`, `net_rx`, `net_tx`, `disk_free`, `disk_total`, `volume`, `muted`, `mic_volume`, `mic_muted`, `brightness`, and optional `osd` (`volume`, `microphone`, or `brightness`) naming a hardware-key change that should receive transient feedback. Unavailable numeric levels are `-1` |
...
| `launcher.list` | `generation` (the number of queries the compositor has answered) and `apps[]` with `id`, `name`, optional `icon` (a `data:` URL, already resolved from the installed themes) and optional `detail` (the entry's keywords, or its main category) — the applications the launcher can start, in answer to `launcher.query`. The scan is the compositor's: the shell is a web page and cannot read `XDG_DATA_DIRS`. The command line itself never crosses the wire; `launcher.launch` names an `id` and carries the list's `generation` back, and the compositor starts what it scanned |
...
| `osk.wanted` | `wanted` — the client with keyboard focus just enabled or disabled a `zwp_text_input_v3`, i.e. asked for (or stopped asking for) an on-screen keyboard. Sent only on the edge, and only as accurately as `sync_osk_wanted` in `input.rs` can notice one without a dedicated callback — see its doc comment. Never sent true at all unless `config.osk` is `"auto"` and the seat has seen a touch device; see `config`'s own row above. The reference shell raises and lowers its own keyboard on this; a shell with a real keyboard attached is free to ignore it |
| `network.update` | `available` (whether NetworkManager answered at all), `wireless`, `enabled` (the radio), `scanning`, `state` (`connected`, `connecting`, `disconnected`, `unknown`), `wired`, optional `ssid`, `access_points[]` with `ssid`, `strength` (0–100), `security` (`""`, `wep`, `wpa`, `wpa2`, `wpa3`, `enterprise`), `known`, `active`, and optional `error` — the wireless radio and what it can see, whenever it changes. One row per network name, strongest first, the one in use above them. Sent only while a picker is open |
| `bluetooth.update` | `available`, `powered`, `discovering`, `adapter` (its name), `devices[]` with `address`, `name`, `icon` (a freedesktop icon name), `paired`, `trusted`, `connected`, optional `rssi` (dBm, only while the adapter can hear it), and optional `error` — the adapter and its devices, whenever they change. Connected first, then paired, then whatever the scan found. Sent only while a picker is open |
...
| `screencast.pick` | `id`, `sources[]` with `kind` (`"output"`, `"window"`, `"all-outputs"`, `"follow-window"` or `"follow-output"`), `label`, `detail`, and `selected` — an application has asked to share the screen, and this is the list to draw with the highlight where the compositor is holding it. Re-sent whole every time the highlight moves. `devices[]` — some of `"keyboard"`, `"mouse"`, `"touchscreen"`, in that order — is present only when the request is the RemoteDesktop portal rather than ScreenCast: the application is asking to *drive* this machine, and the chooser has to ask that question in its own words. A remote request may carry no `sources[]` at all, which is a session that types without watching and a dialog that is a plain yes or no |
| `screencast.pick.done` | `id` — the choice was made or abandoned; take the chooser down |
| `screencast.active` | `active` — whether at least one screencast stream is active. Replayed when the shell reconnects so privacy-sensitive shell policy does not depend on seeing the original start event |
| `shortcuts.pick` | `id`, `app` (what the application calls itself, empty where the portal frontend could not say), `shortcuts[]` with `id`, `description` and `trigger` — an application is asking to be told when these chords are pressed while something else has focus. Its own message rather than a third shape of `screencast.pick` because it is a third question, and there is nothing to highlight: the answer is yes or no to the whole list, so it is sent once rather than re-sent per keypress. `trigger` is spelled the way `binds` spells a chord, because the person answering is comparing it against the keyboard in front of them. See [Global shortcuts](protocols.md#global-shortcuts) |
| `shortcuts.pick.done` | `id` — answered or abandoned; take the dialogue down |
...
| `view.minimized` | `id`, `minimized` — sets compositor-owned minimize state; minimized windows keep their taskbar and layout-tree place |
| `view.focus` | `id` |
| `view.close` | `id` |
...
| `launcher.query` | optional `filter` (absent is the unfiltered list) — the applications the launcher can start, please; what the picker asks when it opens and on every keystroke of its filter. The answer is a `launcher.list` |
| `launcher.launch` | `id` (from the last `launcher.list`) and `generation` (that list's) — start that application. The process is handed an xdg-activation token minted for it, so the window that appears opens focused rather than behind whatever the user moved on to. The picker sends a query on every keystroke and does not wait for the answer before it lets the user press Enter, so a launch that names a generation the compositor has moved past is a row from a list that is no longer the one on screen, and it is refused |
| `mpris.control` | `action` (`play-pause`, `next`, `previous` or `stop`) — a button on the media widget, sent to whichever player the compositor last reported |
 ...
 | `power.action` | `action` (`suspend`, `reboot` or `poweroff`) — the row on the battery picker named that. All three go to logind's `Suspend`, `Reboot` and `PowerOff`, the same `Suspend` call the lid policy makes on a hinge close. They are answered whether or not a battery widget is on the bar: a desk with a lid set to off and no daemon is still a machine. Quitting is not one of them — it is `quit`, because this compositor is the session, and two ways to leave would mean two different things |
| `osk.key` | `keysym` (an X11/XKB keysym), `pressed` — one key of the on-screen keyboard, going down or coming back up. Handed straight to `inject_keysym`, the same call remote-desktop keyboard injection uses; see its doc comment in `input.rs` for why this is key injection rather than `zwp_input_method_v2`'s `commit_string`, which this compositor also has wired up and does not use here. A tap is `true` immediately followed by `false`; a key held down is `true` once and `false` on release, and the seat's own keyboard repeat — already running for every other key on this desktop — does the rest |
| `network.scan` | optional `enabled` (absent means yes) — a picker is open: read the list, scan, and keep sending `network.update`. `false` when it closes, which stops both |
| `network.connect` | `ssid`, optional `passphrase` — join a network. With no passphrase this activates the saved connection, which is what `known` on an access point means; with one, NetworkManager creates a connection and keeps the secret in its own store |
| `network.disconnect` | — leave the wireless network in use. The saved connection is left alone |
| `network.radio` | optional `enabled` (absent toggles) — the wireless radio |
| `bluetooth.scan` | optional `enabled` (absent means yes) — a picker is open: read the device list, start a discovery, and keep sending `bluetooth.update`. `false` when it closes, which stops both |
| `bluetooth.power` | optional `enabled` (absent toggles) — the adapter |
